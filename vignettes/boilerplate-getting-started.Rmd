...
This tutorial builds a complete `boilerplate` workflow, from initial setup to a generated methods section for a scientific manuscript. The example develops one project step by step.

# Scenario: Writing a Psychology Manuscript

Imagine you are writing a manuscript about the relationship between political attitudes and well-being. You want to:
...
When sharing with collaborators, send JSON exports rather than RDS files: JSON carries data only, whereas loading an RDS file can execute code. Exporting a selection with `select_elements` also keeps the shared file small enough for recipients to review before importing.

## 2. Multiple Manuscripts from Same Dataset

Create variants for different manuscripts:
...
Create a reporting function for your team:
...
- ✓ Reuse content across multiple manuscripts
...
- See the [Measures Workflow](boilerplate-measures-workflow.html) vignette for detailed measure documentation

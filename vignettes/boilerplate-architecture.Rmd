...
- RDS reading remains available during the deprecation window, so
  existing databases continue to load via `boilerplate_import()`
- RDS writing is deprecated and emits a warning through
  `boilerplate_save()` and `boilerplate_export()`; it will be removed in
  a future release

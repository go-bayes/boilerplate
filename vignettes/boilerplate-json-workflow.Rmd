...
- **Variable documentation**: The new enhanced functions allow documenting template variables
...
# Create a sample database with enhanced entries
sample_db <- list()

# Add methods with variable documentation
sample_db <- boilerplate_add_entry_enhanced(
  sample_db,
  path = "methods.sampling",
  value = "Participants were randomly selected from {{population}}.",
  variables = list(
    population = "Target population description (e.g., 'university students')"
  )
)
...
sample_db <- boilerplate_add_entry_enhanced(
  ...
  variables = list(
    software = "Statistical software used (e.g., 'R version 4.3.0')"
  )
)
...
```{r health-check}
# Check database health including variable documentation
health <- boilerplate_check_health(sample_db)
print(health)

# List all variables and their documentation status
vars <- boilerplate_list_variables(sample_db)
print(vars)

# Fix issues automatically if needed
if (!health$summary$healthy) {
  health_fixed <- boilerplate_check_health(sample_db, fix = TRUE, quiet = TRUE)
  if (!is.null(health_fixed$summary$issues_fixed) && health_fixed$summary$issues_fixed > 0) {
    sample_db <- attr(health_fixed, "fixed_db")
    message("Fixed ", health_fixed$summary$issues_fixed, " issues")
  }
}
```
...
3. **Document your variables**: Use the enhanced functions to document what each template variable means:
   ```r
   db <- boilerplate_add_entry_enhanced(
     ...
     variables = list(
       ...
       power = "Statistical power percentage (typically 80)"
     )
   )
   ```

4. **Regular health checks**: Run `boilerplate_check_health()` periodically to ensure database quality
...
7. **Export health reports**: Generate health reports for documentation:
   ```r
   boilerplate_health_report(health, file = "db_health_report.txt")
   ```

...
sample_db <- list(
  methods = list(
    sampling = "Participants were randomly selected from {{population}}.",
    analysis = list(
      regression = "We conducted linear regression using {{software}}."
    )
  ),
  measures = list(
    age = list(
...
The package can validate JSON databases against predefined schemas:
...
3. **Validation**: Always validate JSON structure after manual edits

4. **Backup before migration**: Use the `backup = TRUE` option when migrating

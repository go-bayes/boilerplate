...
- **Safer to load**: JSON deserialisation cannot execute code, whereas
  `readRDS()` can through object hooks and class attributes; loading a
  hostile RDS from a collaborator is a real risk that JSON removes

RDS writing is deprecated and will be removed in a future release.
Reading existing RDS databases remains supported, and this vignette
shows the canonical migration path via `boilerplate_migrate_to_json()`.

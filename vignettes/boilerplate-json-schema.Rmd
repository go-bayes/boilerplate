...
### RDS Format (deprecated for writing)
- Binary R object
- Preserves R-specific types that the package does not use
- Not human-readable
- Platform-specific
- Code-execution vector on load: `readRDS()` can invoke attacker code
  through object hooks and class attributes. Writing is deprecated in
  current releases; reading is retained only to support migration of
  existing databases.

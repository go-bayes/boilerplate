...
- Copy-pasting methods sections between manuscripts?
...
1. Maintain consistent methods descriptions across 3 manuscripts
...
# Save the database
boilerplate_save(db, data_path = file.path(project_root, "shared/boilerplate"), confirm = FALSE, quiet = TRUE)
```
...
team_db <- boilerplate_import(
  data_path = file.path(project_root, "shared/boilerplate/lab_boilerplate_v2.json"),
  quiet = TRUE
)
```
...
```{r quality-control}
# Inspect whether all manuscripts use current measure descriptions
papers <- c("study1", "study2", "study3")

for (paper in papers) {
  # Note: This assumes each manuscript has its own boilerplate database
...
library(boilerplate)
db <- boilerplate_import(data_path = "shared/boilerplate", quiet = TRUE)
...
methods <- boilerplate_generate_text(
  ...
  sections = c(
    "recruitment.mturk",
    "statistical.default",  # Using existing path
    "measures.burnout.mbi",
    # Note: measures.engagement.uwes would need to be added
...
results_intro <- boilerplate_generate_text(
  category = "results",
  sections = "main_effect",  # Using existing results section
  global_vars = params,
  db = db
)
...
- **Consistency**: Same descriptions across all manuscripts

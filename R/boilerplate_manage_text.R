...
#' variable substitution. Supports arbitrarily nested hierarchical organisation
...
boilerplate_manage_text <- function(
  ...
  if (!quiet) cli_alert_info("performing {action} action on {category} database")

  # validate name for actions that require it
  if (action %in% c("add", "update", "remove", "get") && is.null(name)) {
    if (!quiet) cli_alert_danger("the {action} action requires a name parameter")
  ...
  if (action %in% c("add", "update") && is.null(value)) {
    if (!quiet) cli_alert_danger("the {action} action requires a value parameter")
    stop(paste("The", action, "action requires a value parameter."))
  }

  # load default database if not provided
  if (is.null(db)) {
    if (!quiet) cli_alert_info("loading {category} database")
    db <- load_text_db(category, text_path, file_name, quiet)
  }

  # input validation: ensure db is a list
  if (!is.list(db)) {
    if (!quiet) cli_alert_danger("database must be a list")
  ...
  if (!is.null(name) && grepl("\\.", name)) {
    ...
    if (!quiet) cli_alert_info("handling nested path: {paste(path_parts, collapse = '.')}")

    if (action == "list") {
      # for list action, navigate to the requested folder
      if (!quiet) cli_alert_info("listing nested folder")
      return(get_nested_folder(db, path_parts))
    } else if (action == "get") {
      # for get action, retrieve and process the text
      if (!quiet) cli_alert_info("retrieving and processing nested text")
      ...
      if (!quiet) cli_alert_info("{action} nested text entry")
      ...
      if (auto_sort && action %in% c("add", "update")) {
        if (!quiet) cli_alert_info("auto-sorting database")
        db <- sort_db_recursive(db)
      }

      if (!quiet) cli_alert_success("successfully {action}ed nested text entry: {name}")
      return(db)
    }
  } else {
    ...
    if (action == "list") {
      if (!is.null(name)) {
        ...
        if (!(name %in% names(db))) {
          if (!quiet) cli_alert_danger("text entry {name} not found")
          stop(paste("Text entry", name, "not found."))
        }
        if (!quiet) cli_alert_success("retrieving text entry {name}")
        return(db[[name]])
      }
      if (!quiet) cli_alert_success("listing complete {category} database")
      return(db)
    } else if (action == "add") {
      if (name %in% names(db)) {
        if (!quiet) cli_alert_danger("text entry {name} already exists")
        stop("text entry already exists")
      }
      if (!quiet) cli_alert_info("adding text entry {name}")
      db[[name]] <- value
      ...
      if (!quiet) cli_alert_success("successfully added text entry: {name}")
    } else if (action == "update") {
      if (!(name %in% names(db))) {
        if (!quiet) cli_alert_danger("text entry {name} does not exist")
        stop("text entry does not exist")
      }
      if (!quiet) cli_alert_info("updating text entry {name}")
      db[[name]] <- value
      ...
      if (!quiet) cli_alert_success("successfully updated text entry: {name}")
    } else if (action == "remove") {
      if (!(name %in% names(db))) {
        if (!quiet) cli_alert_danger("text entry {name} does not exist")
        stop("text entry does not exist")
      }
      if (!quiet) cli_alert_info("removing text entry {name}")
      db[[name]] <- NULL

      if (!quiet) cli_alert_success("successfully removed text entry: {name}")
    } else if (action == "get") {
      if (!(name %in% names(db))) {
        if (!quiet) cli_alert_danger("text entry {name} does not exist")
        stop("text entry does not exist")
      ...
      if (!quiet) cli_alert_info("retrieving text entry {name}")
      text <- db[[name]]

      # apply template substitution
      if (!quiet) cli_alert_info("applying template substitution")
      result <- apply_template_vars(text, template_vars, warn_missing)

      if (!quiet) cli_alert_success("successfully retrieved and processed text entry: {name}")
      return(result)
    } else if (action == "save") {
      if (!quiet) cli_alert_info("saving {category} database")
      result <- save_text_db(db, category, text_path, file_name, quiet)

      if (result && !quiet) {
        cli_alert_success("successfully saved {category} database")
      }
...
#'
#' @noRd
load_text_db <- function(category, text_path = NULL, file_name = NULL, quiet = FALSE) {
  # determine file path
  file_path <- get_db_file_path(category, text_path, file_name)

  if (!quiet) cli_alert_info("attempting to load {category} database from {file_path}")

  # check if file exists
  if (file.exists(file_path)) {
    tryCatch({
      db <- readRDS(file_path)
      if (!quiet) cli_alert_success("{category} text database loaded from {file_path}")
      return(db)
    }, error = function(e) {
      if (!quiet) cli_alert_warning("error loading database: {e$message}")
      # fall back to defaults if loading fails
    })
  } else if (!quiet) {
    cli_alert_warning("database file not found at {file_path}, using defaults")
  }
...
#'
#' @noRd
save_text_db <- function(db, category, text_path = NULL, file_name = NULL, quiet = FALSE) {
  # determine file path
  file_path <- get_db_file_path(category, text_path, file_name)
  ...
  if (!dir.exists(dir_path)) {
    ...
    if (!quiet) cli_alert_success("created directory: {dir_path}")
  }

  # check if file exists and warn
  if (file.exists(file_path) && !quiet) {
    cli_alert_warning("overwriting existing {category} database file: {file_path}")
  }

  # save database
  tryCatch({
    saveRDS(db, file = file_path)
    if (!quiet) cli_alert_success("{category} text database saved to {file_path}")
    return(TRUE)
  }, error = function(e) {
    if (!quiet) cli_alert_danger("error saving database: {e$message}")
    return(FALSE)
  })
}

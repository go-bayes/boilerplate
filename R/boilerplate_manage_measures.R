...
boilerplate_manage_measures <- function(
  ...
  if (!quiet) cli_alert_info("performing {action} action on measures database")
  ...
  if (action %in% c("add", "update") && is.null(measure)) {
    if (!quiet) cli_alert_danger("the {action} action requires a measure parameter")
    stop(paste("The", action, "action requires a measure parameter."))
  }

  # load default database if not provided
  if (is.null(db)) {
    if (!quiet) cli_alert_info("loading measures database")
    db <- load_measures_db(measures_path, file_name, quiet)
  }
  ...
  if (!is.null(name) && grepl("\\.", name)) {
    ...
    if (action == "list") {
      ...
      if (!quiet) cli_alert_info("retrieving nested measure")
      ...
      if (!quiet) cli_alert_info("{action} nested measure")
      ...
      if (!quiet) cli_alert_success("successfully {action}ed nested measure: {name}")
      return(db)
    }
  } else {
    ...
    if (action == "list") {
      if (!is.null(name)) {
        ...
        if (!(name %in% names(db))) {
          if (!quiet) cli_alert_danger("measure {name} not found")
          stop(paste("Measure", name, "not found."))
        }
        if (!quiet) cli_alert_success("retrieving measure {name}")
        return(db[[name]])
      }
      if (!quiet) cli_alert_success("listing complete database")
      return(db)
    } else if (action == "add") {
      if (name %in% names(db)) {
        if (!quiet) cli_alert_danger("measure {name} already exists")
      ...
      if (!quiet) cli_alert_success("successfully added measure: {name}")
    } else if (action == "update") {
      if (!(name %in% names(db))) {
        if (!quiet) cli_alert_danger("measure {name} does not exist")
      ...
      if (!quiet) cli_alert_success("successfully updated measure: {name}")
      ...
      if (!quiet) cli_alert_success("successfully removed measure: {name}")
    } else if (action == "get") {
      if (!(name %in% names(db))) {
        if (!quiet) cli_alert_danger("measure {name} does not exist")
        stop("measure does not exist")
      ...
      result <- save_measures_db(db, measures_path, file_name, quiet)

      if (result && !quiet) {
        cli_alert_success("successfully saved measures database")
      }
...
#'
#' @noRd
load_measures_db <- function(measures_path = NULL, file_name = NULL, quiet = FALSE) {
  # determine file path
  file_path <- get_db_file_path("measures", measures_path, file_name)

  if (!quiet) cli_alert_info("attempting to load database from {file_path}")

  # check if file exists
  if (file.exists(file_path)) {
    tryCatch({
      db <- readRDS(file_path)
      if (!quiet) cli_alert_success("measures database loaded from {file_path}")
      return(db)
    }, error = function(e) {
...
#'
#' @noRd
save_measures_db <- function(db, measures_path = NULL, file_name = NULL, quiet = FALSE) {
  # determine file path
  file_path <- get_db_file_path("measures", measures_path, file_name)
  ...
  if (file.exists(file_path) && !quiet) {
    cli_alert_warning("overwriting existing measures database file: {file_path}")
  }

  # save database
  tryCatch({
    saveRDS(db, file = file_path)
    if (!quiet) cli_alert_success("measures database saved to {file_path}")
    return(TRUE)
  }, error = function(e) {

...
#'
#' @return A list with class "boilerplate_health" containing:
#'   - `summary`: Overall health summary
#'   - `issues`: Detailed list of issues found
#'   - `stats`: Database statistics
#'   - `fixed`: If fix=TRUE, what was fixed
#'
#' @examples
#' \dontrun{
#' # Check database health
...
#'
#' @export
boilerplate_check_health <- function(db, fix = FALSE, quiet = FALSE) {
  ...
  for (path in paths_to_check) {
    ...
    if (!is.null(text_content)) {
      ...
      if (length(vars) > 0) {
        ...
        var_info <- tryCatch(
          boilerplate_get_variables(db, path),
          error = function(e) list(documented = character(0))
        )
        
        documented <- intersect(vars, var_info$documented)
        stats$documented_variables <- stats$documented_variables + length(documented)
        
        undocumented <- setdiff(vars, documented)
        if (length(undocumented) > 0) {
          orphaned_vars[[path]] <- undocumented
        }
...
#'
#' Creates a detailed health report as a text file or returns as string.
#'
#' @param health A boilerplate_health object from boilerplate_check_health()
#' @param file Path to save the report. If NULL, returns as string.
#'
#' @return If file is NULL, returns the report as a character string.
#'   Otherwise, invisibly returns the file path.
#'
#' @export
boilerplate_health_report <- function(health, file = NULL) {
  if (!inherits(health, "boilerplate_health")) {
    stop("Input must be a boilerplate_health object from boilerplate_check_health()")
  }
  
  # Capture the print output
  report <- utils::capture.output(print(health))
  
  # Add detailed information
  report <- c(report, "", "Detailed Issue Information", "=========================", "")
  
  # Add all orphaned variables
  if (!is.null(health$issues$orphaned_variables)) {
    report <- c(report, "All Undocumented Variables:", "")
    
    vars_by_name <- list()
    for (path in names(health$issues$orphaned_variables$details)) {
      for (var in health$issues$orphaned_variables$details[[path]]) {
  ...
  if (is.null(file)) {
    ...
    cli::cli_alert_success("Health report saved to {file}")
    invisible(file)
  }
}
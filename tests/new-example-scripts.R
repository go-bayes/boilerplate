...
measures_db$alcohol_intensity
cat(exposure_text)

# Generate text for psychological outcome variables
psychological_text <- boilerplate_measures_text(
  variable_heading = "Psychological Outcomes",
  variables = c("alcohol_frequency", "alcohol_intensity"),
  heading_level = 3,
  subheading_level = 4,
  print_waves = TRUE,
...
cat(psychological_text)


# Example: Combining sections in a research report
cat("## Variables\n\n")
cat(exposure_text)
cat("\n### Outcome Variables\n\n")
cat(psychological_text)
...
# 3. Generate methods text for participant selection
...
# 4. Generate measures text using the simplified function
# Exposure variable
exposure_text <- boilerplate_measures_text(
  variable_heading = "Exposure Variable",
  variables = "perfectionism",
...
# Outcome variables (psychological domain)
...
# Outcome variables (health domain)
...
cat(temp_methods_db$sample$nzavs$default)
...
cat(temp_methods_db$identification_assumptions$standard)

# save using the margot package
my_methods_path = here::here("data")
margot::here_save(temp_methods_db, "temp_methods_db", my_methods_path)
...
temp_methods_db <- boilerplate_manage_text(
  category = "methods",
  action = "remove",
  name = "outcome_domains.flourishing_2025",
  value = method_outcomes_domains_health,
  db = temp_methods_db
)
...
cat(temp_methods_db$confounding_control$vanderweele)

# save
margot::here_save(temp_methods_db, "temp_methods_db", my_methods_path)

cat(temp_methods_db$outcome_domains$flourishing_2025)
...
margot::here_save(temp_methods_db, "temp_methods_db", methods_path)
